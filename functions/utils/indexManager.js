...
export async function readIndex(context, options = {}) {
    try {
        ...
        // 归属过滤：管理员看全部；普通用户看「自己 + 所有加入的组 + 被好友授权的盘」
        if (scope && !scope.isAdmin) {
            const grantors = scope.grantorIds || [];
            const groupIds = scope.groupIds || [];
            const singleGroupId = scope.groupId || '';
            filteredFiles = filteredFiles.filter(file => {
                const owner = file.metadata.OwnerId || '';
                const fileGroup = file.metadata.GroupId || '';
                if (owner && owner === scope.userId) return true;
                if (fileGroup && (groupIds.includes(fileGroup) || fileGroup === singleGroupId)) return true;
                if (owner && grantors.includes(owner)) return true;
                return false;
            });
        }
            ...
            filteredFiles = filteredFiles.filter(file => {
                ...
                return fileTypeArr.some(ft => {
                    if (ft === 'image') {
                        ...
                        return /\.(pdf|doc|docx|xls|xlsx|ppt|pptx|txt|csv|md|rtf)$/i.test(file.metadata.FileName || '');
                    } else if (ft === 'other') {
                        return !mimeType.startsWith('image/') && 
