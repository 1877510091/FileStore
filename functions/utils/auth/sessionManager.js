...
export async function createSession(env, authType, username = '') {
    ...
    const rawMaxAgeDays = authType === 'admin'
        ? (accessConfig.adminSessionMaxAge ?? 90)
        : (accessConfig.userSessionMaxAge ?? 90);
...
export async function destroySessionsByAuthType(env, authType) {
    ...
    // user 会话在 other_data 中还有一条「登录数据」映射（manage@session@user_<token>），
    // 凭证被清后这些映射会成为孤儿，一并清理
    if (authType === 'user' && env.img_d1 && typeof env.img_d1.prepare === 'function') {
        try {
            await env.img_d1.prepare("DELETE FROM other_data WHERE key LIKE 'manage@session@user_%'").run();
        } catch (e) { /* 忽略 */ }
    }
...
/**
 * 踢掉某个用户的全部会话（管理员禁用/删除用户时调用，实现实时下线）
 * 会话凭证在 settings 表（manage@session@<token>），登录数据在 other_data（manage@session@user_<token>，value 含 userId）
 * @returns {Promise<number>} 清除的会话数量
 */
export async function destroyUserSessionsById(env, userId) {
    if (!userId) return 0;
    const db = getDatabase(env);
    const d1 = env.img_d1;
    if (!d1 || typeof d1.prepare !== 'function') return 0;

    let destroyed = 0;
    const rows = await d1.prepare("SELECT key, value FROM other_data WHERE key LIKE 'manage@session@user_%'").all();
    for (const row of (rows.results || [])) {
        let v;
        try { v = JSON.parse(row.value); } catch (e) { continue; }
        if (!v || v.userId !== userId) continue;
        const token = row.key.slice('manage@session@user_'.length);
        if (token) {
            try { await db.delete(SESSION_PREFIX + token); } catch (e) { /* 忽略 */ }
        }
        try { await d1.prepare('DELETE FROM other_data WHERE key = ?').bind(row.key).run(); } catch (e) { /* 忽略 */ }
        destroyed++;
    }
    return destroyed;
}
