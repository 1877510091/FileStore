...
export async function onRequest(context) {
    ...
    if (request.method === 'POST') {
        ...
        if (newSettings.rateLimit) {
            settings.rateLimit = {
                enabled: newSettings.rateLimit.enabled ?? true,
                maxAttempts: newSettings.rateLimit.maxAttempts || 5,
                windowSeconds: newSettings.rateLimit.windowSeconds || 60,
            }
        }
...
export async function getSecurityConfig(db, env) {
    ...
    const access = {
        ...
        userSessionMaxAge: normalizeSessionMaxAgeDays(kvAccess.userSessionMaxAge ?? 90),
        adminSessionMaxAge: normalizeSessionMaxAgeDays(kvAccess.adminSessionMaxAge ?? 90),
    }
    ...
    // 速率限制
    const kvRateLimit = settingsKV.rateLimit || {}
    settings.rateLimit = {
        enabled: kvRateLimit.enabled ?? true,
        maxAttempts: kvRateLimit.maxAttempts || 5,
        windowSeconds: kvRateLimit.windowSeconds || 60,
    }
