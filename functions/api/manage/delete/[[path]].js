...
export async function onRequest(context) {
    ...
    if (folder === 'true') {
        try {
            ...
            // 使用队列存储需要处理的文件夹
            const rootPath = params.path.split(',').join('/');
            const folderQueue = [{
                path: rootPath
            }];
            ...
            if (deletedFiles.length > 0) {
                ...
                await logRequestActivity(env, request, 'delete', {
                    fileId: rootPath,
                    fileName: rootPath,
                    details: `删除文件夹，共 ${deletedFiles.length} 个文件`
                });
            }
    ...
    try {
        ...
        if (!success) {
            ...
            await logRequestActivity(env, request, 'delete', { fileId, fileName: fileId.split('/').pop() });
        }
