...
export async function onRequest(context) {
    ...
    try {
        ...
        if (deleted.length > 0) {
            ...
            await logRequestActivity(context.env, context.request, 'delete', {
                fileId: deleted[0],
                fileName: deleted.length === 1 ? deleted[0].split('/').pop() : `${deleted.length} 个文件`,
                details: `批量删除 ${deleted.length} 个文件`,
            });
        }
