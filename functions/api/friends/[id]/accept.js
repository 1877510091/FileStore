export async function onRequest(context) {
    ...
    if (request.method === 'POST') {
        try {
            const existing = await db.prepare('SELECT * FROM friendships WHERE id = ? AND friend_id = ?').bind(friendshipId, currentUserId).first();
            if (!existing) return new Response(JSON.stringify({ code: -1, message: '请求不存在' }), { status: 404, headers: { 'content-type': 'application/json' } });
            await db.prepare("UPDATE friendships SET status = 'accepted' WHERE id = ?").bind(friendshipId).run();
            return new Response(JSON.stringify({ code: 0, message: '已接受好友请求' }), { headers: { 'content-type': 'application/json' } });
