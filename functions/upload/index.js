...
export async function onRequest(context) {  // Contents of context object
    ...
    // 记录上传者归属（供多用户/组隔离使用）
    context.uploader = await getSessionUser(env, request);

    // 获得上传IP
    const uploadIp = getUploadIp(request);    // 判断上传ip是否被封禁
...
async function processFileUpload(context, formdata = null) {
    ...
    const metadata = {
        ...
        Tags: [],
        OwnerId: context.uploader ? context.uploader.id : '',
        OwnerName: context.uploader ? (context.uploader.displayName || context.uploader.username) : '',
        GroupId: url.searchParams.get('groupId') || (context.uploader ? (context.uploader.groupId || '') : '')
    };
...
async function uploadFileToTelegram(context, fullId, metadata, fileExt, fileName, fileType, returnLink) {
    ...
    if (fileSize > CHUNK_SIZE) {
        // 浏览器直传，不受 Worker 子请求上限约束
        return await uploadLargeFileToTelegram(context, file, fullId, metadata, fileName, fileType, returnLink, tgBotToken, tgChatId, tgChannel, true);
    }
