...
/* ======= 单个大文件分块上传到Telegram ======= */
/* isDirectUpload=true 时跳过 MAX_PARTS 检查 —— 浏览器分片走独立请求，不受 Worker 子请求上限约束 */
export async function uploadLargeFileToTelegram(context, file, fullId, metadata, fileName, fileType, returnLink, tgBotToken, tgChatId, tgChannel, isDirectUpload = false) {
    ...
    // 文件转通道时受 Cloudflare 免费版「单次请求最多 50 个外部子请求」限制：
    //   N 段分片 + 1 次取源文件 = N+1，所以 MAX_PARTS = 48（48+1=49 < 50）。
    // 直传走浏览器分片（每片独立 Worker 请求），不受此限制。
    if (!isDirectUpload) {
        const tgType = channelTypeOf('telegram') || {};
        const MAX_PARTS = tgType.maxParts || 48;
        const MAX_BYTES = tgType.maxFileBytes || MAX_PARTS * CHUNK_SIZE;
        if (totalChunks > MAX_PARTS) {
            return createResponse(
                `Error: File too large for channel transfer (exceeds ${Math.round(MAX_BYTES / 1024 / 1024)}MB limit, caused by Cloudflare 50 sub-request cap)`,
                { status: 413 }
            );
        }
    }
